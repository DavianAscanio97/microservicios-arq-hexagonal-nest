...
export class DrizzleUserRepository implements UserRepository {
    ...
    */
    async create(user: User): Promise<void> {
        console.log('OE PARCE UN SALUDO A TODOS DESDE USERS')
    ...
    */
    async publishEvents(user: User, extra?: object): Promise<void> {
        const events = user.pullDomainEvents();
        for (const event of events) {
            if (event.getEventName() === UserEventsEnum.CREATED_USERS) await this._createdUserProducer.emitUserCreatedEvent(user, extra);
            if (event.getEventName() === UserEventsEnum.UPDATED_USERS) await this._createdUserProducer.emitUserCreatedEvent(user, extra);
            if (event.getEventName() === UserEventsEnum.DELETED_USERS) await this._createdUserProducer.emitUserCreatedEvent(user, extra);
        }
    }
